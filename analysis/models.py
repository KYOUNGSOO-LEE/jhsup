...
class ibsi(models.Model):
    ...
    grade = models.DecimalField(blank = True, null = True, decimal_places = 3, max_digits = 4) # 등급(대학별 환산등급)
    myscore = models.DecimalField(blank = True, null = True, decimal_places = 3, max_digits = 4) # 대학별 환산점수
    ...
    first_step = models.CharField(max_length = 5, blank = True, null = True) # 1단계 합격/불합격
    final_step = models.CharField(max_length = 5) # 최종 합격/불합격
    fail_reason = models.CharField(max_length = 10, blank = True, null = True) # 불합격 사유
    candidate_rank = models.IntegerField(blank=True, null=True)  # 최초 후보순위
    num_students = models.IntegerField(blank=True, null = True)  # 모집인원
